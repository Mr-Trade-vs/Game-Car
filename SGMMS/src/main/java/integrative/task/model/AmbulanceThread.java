...
/**
 * Thread class for Ambulance emergency vehicles
 */
public class AmbulanceThread implements Runnable {
    private final AmbulanceCar ambulanceCar;
    ...
    private final String color = "white"; // Ambulance color for console visualization

    // Algorithm selection flag (shared with PoliceCar)
    private boolean useBFS = false;

    // Mission completion tracking
    private long arrivalTime = 0; // Time when ambulance arrived at accident

    public AmbulanceThread(AmbulanceCar ambulanceCar, Graph<Intersection> cityGraph, 
                    Intersection startIntersection, String name) {
        this.ambulanceCar = ambulanceCar;
        ...
        if (startIntersection != null) {
            ...
            if (this.currentNodeGraph != null) {
                ambulanceCar.setPosition(currentNodeGraph.getX(), currentNodeGraph.getY());
                System.out.println(getColoredText("Ambulance " + name + " stationed at " + startIntersection.getName()));
            } else {
                // Fallback to getting node from graph if topLeft is null
                this.currentNodeGraph = cityGraph.getNode(startIntersection);
                if (this.currentNodeGraph != null) {
                    ambulanceCar.setPosition(currentNodeGraph.getX(), currentNodeGraph.getY());
                    System.out.println(getColoredText("Ambulance " + name + " stationed at " + startIntersection.getName() + " (fallback)"));
                }
            }
        }
    }
    ...
    @Override
    public void run() {
        System.out.println(getColoredText("Ambulance " + name + " thread started"));

        while (running && currentNodeGraph != null) {
            try {
                if (hasDestination && destinationNodeGraph != null) {
                    // Debug output for destination
                    System.out.println(getColoredText("Ambulance " + name + 
                    ...
                    if (useBFS) {
                        // Use BFS algorithm
                        System.out.println(getColoredText("Ambulance " + name + " using BFS algorithm"));
                        ...
                        System.out.println(getColoredText("Ambulance " + name + " using Dijkstra algorithm"));
                    ...
                    if (!path.isEmpty() && path.size() > 1) {
                        ...
                        // Print ambulance's current position in the console with color
                        System.out.println(getColoredText("Ambulance " + name + 
                        ...
                        // Move the ambulance with animation
                        ambulanceCar.moveTo(nextNodeGraph.getX(), nextNodeGraph.getY(), direction, () -> {
                            System.out.println(getColoredText("Ambulance " + name + 
                        ...
                        if (currentNodeGraph.equals(destinationNodeGraph)) {
                            System.out.println(getColoredText("Ambulance " + name + 
                                              " reached accident location at " + destinationNodeGraph.getData().getName()));
                        ...
                        if (!availableDirections.isEmpty()) {
                            ...
                            if (bestDirection != null) {
                                ...
                                ambulanceCar.moveTo(nextNodeGraph.getX(), nextNodeGraph.getY(), bestDirection, () -> {
                                    System.out.println(getColoredText("Ambulance " + name + 
                                                      " arrived at " + finalNextNodeGraph.getData().getName() +
                    ...
                    if (currentTime - arrivalTime >= 2000) {
                        ...
                        // Signal that this ambulance is done and can be removed
                        running = false;
                    }
                } else {
                    // No destination, ambulance remains stationary
                    // System.out.println(getColoredText("Ambulance " + name + " waiting for accident..."));
                }
                ...
                System.out.println(getColoredText("Ambulance " + name + " thread interrupted"));
                running = false;
            }
        }

        System.out.println(getColoredText("Ambulance " + name + " thread finished"));
    }

    private String getColoredText(String text) {
        return "\u001B[37m" + text + "\u001B[0m"; // White color for ambulance
    }
    ...
    private String determineDirection(NodeGraph<Intersection> source, NodeGraph<Intersection> target) {
        ...
        for (String direction : cityGraph.getDirections(source)) {
            ...
            if (neighbor != null && neighbor.equals(target)) {
                System.out.println(getColoredText("Ambulance " + name + " found direction: " + direction + 
        ...
        System.out.println(getColoredText("WARNING: Ambulance " + name + 
        ...
        if (!directions.isEmpty()) {
            ...
            System.out.println(getColoredText("Ambulance " + name + " selected random direction: " + randomDirection));
